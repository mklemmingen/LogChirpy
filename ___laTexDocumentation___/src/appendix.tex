...
\begin{itemize}
    ...
    \item \textbf{RAM:} Minimum 3GB for optimal machine learning performance
...
\subsection{Machine Learning Model Specifications}
...
\subsubsection{Audio Classification}
\begin{itemize}
...
\begin{lstlisting}[style=javascript, caption=Package Versions, label=lst:package_versions]
{
  "expo": "~51.0.0",
  "react": "18.2.0",
  "react-native": "0.74.0",
  ...
  "@infinitered/react-native-mlkit-object-detection": "^0.7.0",
  "@infinitered/react-native-mlkit-image-labeling": "^0.7.0",
  "expo-sqlite": "~14.0.0",
  "firebase": "^10.7.0"
}
\end{lstlisting}
...
\begin{lstlisting}[style=javascript, caption=Development Scripts, label=lst:dev_scripts]
...
\subsection{SQLite Database Schema Implementation}
\begin{lstlisting}[style=javascript, caption=Database Schema, label=lst:db_schema]
...
\subsection{Firebase Firestore Data Collections}
\begin{lstlisting}[style=javascript, caption=Firestore Collections, label=lst:firestore_collections]
...
\subsection{Machine Learning Pipeline Performance Analysis}
\begin{table}[H]
\centering
...
\subsection{Application Configuration Parameters}
\begin{lstlisting}[style=javascript, caption=Configuration Options, label=lst:config_options]
export const Config = {
...
\subsection{Common System Issues and Resolution Procedures}

\subsubsection{Machine Learning Pipeline Initialization Failure}
...
\subsubsection{Audio Classification System Malfunction}
...
\subsubsection{System Performance Degradation}
...
\subsection{Diagnostic Command Interface}
\begin{lstlisting}[style=javascript, caption=Debug Commands, label=lst:debug_commands]
...
\subsection{Android Application Package Build Process}
\begin{lstlisting}[style=javascript, caption=Android Build Process, label=lst:android_build]
...
\subsection{iOS Application Build Framework}
\begin{lstlisting}[style=javascript, caption=iOS Build Process, label=lst:ios_build]
# Dependency installation procedure
npm install --legacy-peer-deps
...
\subsection{Open Source License Compliance}
\begin{itemize}
...
\begin{itemize}
    \item \textbf{BirDex Database:} Compiled from comprehensive ornithological sources
    \item \textbf{Wikipedia:} Species information and taxonomic imagery
    \item \textbf{Macaulay Library:} Reference media for avian species validation
    \item \textbf{OpenStreetMap:} Cartographic data and geospatial services
\end{itemize}

\subsection{External Technical Contributions}
\begin{itemize}
    \item \textbf{BirdNET Team:} Audio classification model framework provision
    \item \textbf{Infinite Red:} Reliable MLKit React Native package maintenance
    \item \textbf{Marc Wannenmacher:} React Native Vision Camera library development
    \item \textbf{whoBIRD Project:} TensorFlow Lite implementation reference architecture
\end{itemize}
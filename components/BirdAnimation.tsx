import React, { useEffect, useState, useRef, useCallback } from 'react';
import { View, StyleSheet, Dimensions, TouchableWithoutFeedback } from 'react-native';
import { Audio } from 'expo-av';
import { useFocusEffect } from '@react-navigation/native';
import Animated, {
    useSharedValue,
    withRepeat,
    withTiming,
    withSequence,
    useAnimatedStyle,
    useDerivedValue,
    Easing,
    runOnJS,
} from 'react-native-reanimated';

// Types
interface BirdConfig {
    speed: { min: number; max: number };
    waveAmplitude: { min: number; max: number };
    waveFrequency: { min: number; max: number };
    scale: { min: number; max: number };
    frameRate: number;
}

interface Bird {
    id: string;
    sprite: any;
    spriteIndex: number;
    frameIndex: number;
    speed: number;
    scale: number;
    opacity: number;
    depth: number;
    zIndex: number;
    baseY: number;
    waveAmplitude: number;
    waveFrequency: number;
    waveOffset: number;
    frameRate: number;
    windSensitivity: number;
    // Reanimated values
    x: Animated.SharedValue<number>;
    y: Animated.SharedValue<number>;
    waveY: Animated.SharedValue<number>;
}

const birdSprites = [
    require('@/assets/birds/spritesheet_magpie.png'),
    require('@/assets/birds/spritesheet_house finch.png'),
    require('@/assets/birds/spritesheet_white_dove.png'),
    require('@/assets/birds/spritesheet_wood_thrush.png'),
];

const birdSounds = [
    require('@/assets/birds/bird1.mp3'),
    require('@/assets/birds/bird2.mp3'),
    require('@/assets/birds/bird3.mp3'),
];

const SPECIES_CONFIG: Record<string, BirdConfig> = {
    magpie: {
        speed: { min: 0.4, max: 0.8 },
        waveAmplitude: { min: 25, max: 45 },
        waveFrequency: { min: 0.0008, max: 0.0015 },
        scale: { min: 0.8, max: 1.2 },
        frameRate: 250,
    },
    finch: {
        speed: { min: 0.6, max: 1.2 },
        waveAmplitude: { min: 15, max: 30 },
        waveFrequency: { min: 0.0012, max: 0.002 },
        scale: { min: 0.5, max: 0.8 },
        frameRate: 200,
    },
    dove: {
        speed: { min: 0.3, max: 0.6 },
        waveAmplitude: { min: 10, max: 20 },
        waveFrequency: { min: 0.0006, max: 0.001 },
        scale: { min: 0.9, max: 1.3 },
        frameRate: 300,
    },
    thrush: {
        speed: { min: 0.5, max: 0.9 },
        waveAmplitude: { min: 20, max: 35 },
        waveFrequency: { min: 0.001, max: 0.0018 },
        scale: { min: 0.7, max: 1.0 },
        frameRate: 220,
    },
};

const { width: screenWidth, height: screenHeight } = Dimensions.get('window');

interface EnhancedBirdAnimationProps {
    numberOfBirds?: number;
}

const EnhancedBirdAnimation: React.FC<EnhancedBirdAnimationProps> = ({
                                                                         numberOfBirds = 5
                                                                     }) => {
    const [birds, setBirds] = useState<Bird[]>([]);
    const [sounds, setSounds] = useState<Audio.Sound[]>([]);

    const isComponentMounted = useRef(true);
    const isScreenFocused = useRef(true);
    const frameIntervalsRef = useRef<NodeJS.Timeout[]>([]);

    // Wind effect using proper Reanimated pattern
    const windStrength = useSharedValue(0);

    // ✅ Proper way to handle shared value changes
    const windOffset = useDerivedValue(() => {
        return windStrength.value;
    });

    // Load sounds on mount
    useEffect(() => {
        const loadSounds = async () => {
            try {
                const loaded = await Promise.all(
                    birdSounds.map(async (file) => {
                        const { sound } = await Audio.Sound.createAsync(file, { shouldPlay: false });
                        return sound;
                    })
                );
                if (isComponentMounted.current) {
                    setSounds(loaded);
                }
            } catch (error) {
                console.warn('Error loading bird sounds:', error);
            }
        };
        loadSounds();

        return () => {
            isComponentMounted.current = false;
            sounds.forEach((s) => s?.unloadAsync?.());
        };
    }, []);

    // Wind animation using proper Reanimated pattern
    useEffect(() => {
        if (isScreenFocused.current) {
            windStrength.value = withRepeat(
                withSequence(
                    withTiming(1, {
                        duration: 8000,
                        easing: Easing.inOut(Easing.sin),
                    }),
                    withTiming(-1, {
                        duration: 6000,
                        easing: Easing.inOut(Easing.sin),
                    }),
                    withTiming(0, {
                        duration: 4000,
                        easing: Easing.inOut(Easing.sin),
                    }),
                ),
                -1,
                false
            );
        }
    }, []);

    // Handle screen focus
    useFocusEffect(
        useCallback(() => {
            console.log("[BirdAnimation] Screen focused");
            isScreenFocused.current = true;

            return () => {
                console.log("[BirdAnimation] Screen unfocused");
                isScreenFocused.current = false;
            };
        }, [])
    );

    const getSpeciesConfig = (spriteIndex: number): BirdConfig => {
        const speciesKeys = Object.keys(SPECIES_CONFIG);
        const speciesKey = speciesKeys[spriteIndex % speciesKeys.length];
        return SPECIES_CONFIG[speciesKey];
    };

    const createBird = useCallback((index: number): Bird => {
        const spriteIndex = Math.floor(Math.random() * birdSprites.length);
        const config = getSpeciesConfig(spriteIndex);

        const speed = config.speed.min + Math.random() * (config.speed.max - config.speed.min);
        const waveAmplitude = config.waveAmplitude.min + Math.random() * (config.waveAmplitude.max - config.waveAmplitude.min);
        const waveFrequency = config.waveFrequency.min + Math.random() * (config.waveFrequency.max - config.waveFrequency.min);
        const scale = config.scale.min + Math.random() * (config.scale.max - config.scale.min);

        const depth = Math.random();
        const opacity = 0.3 + depth * 0.7;
        const finalScale = scale * (0.6 + depth * 0.4);
        const baseY = 50 + Math.random() * (screenHeight - 150);

        return {
            id: Math.random().toString(36).substring(7),
            sprite: birdSprites[spriteIndex],
            spriteIndex,
            frameIndex: 0,
            speed,
            scale: finalScale,
            opacity,
            depth,
            zIndex: Math.floor(depth * 10),
            baseY,
            waveAmplitude,
            waveFrequency,
            waveOffset: Math.random() * Math.PI * 2,
            frameRate: config.frameRate + Math.random() * 100 - 50,
            windSensitivity: 0.3 + Math.random() * 0.7,
            // Reanimated shared values
            x: useSharedValue(-64 * finalScale),
            y: useSharedValue(baseY),
            waveY: useSharedValue(0),
        };
    }, []);

    const startBirdAnimation = useCallback((bird: Bird) => {
        if (!isComponentMounted.current || !isScreenFocused.current) return;

        const duration = 15000 / bird.speed;
        const totalDistance = screenWidth + 128 * bird.scale;

        // Reset position
        bird.x.value = -64 * bird.scale;
        bird.waveY.value = 0;

        // Start flight animation
        bird.x.value = withTiming(totalDistance, {
            duration,
            easing: Easing.linear,
        }, (finished) => {
            if (finished && isComponentMounted.current && isScreenFocused.current) {
                // Restart after delay
                setTimeout(() => {
                    if (isComponentMounted.current && isScreenFocused.current) {
                        // Update Y position for next flight
                        bird.baseY = 50 + Math.random() * (screenHeight - 150);
                        bird.y.value = bird.baseY;
                        startBirdAnimation(bird);
                    }
                }, 1000 + Math.random() * 3000);
            }
        });

        // Wave animation
        bird.waveY.value = withRepeat(
            withSequence(
                withTiming(bird.waveAmplitude, {
                    duration: 2000,
                    easing: Easing.inOut(Easing.sin),
                }),
                withTiming(-bird.waveAmplitude, {
                    duration: 2000,
                    easing: Easing.inOut(Easing.sin),
                }),
            ),
            -1,
            false
        );
    }, []);

    // Setup birds
    useEffect(() => {
        console.log(`[BirdAnimation] Setting up ${numberOfBirds} birds`);

        // Clear existing intervals
        frameIntervalsRef.current.forEach(interval => clearInterval(interval));

        const createdBirds = Array.from({ length: numberOfBirds }, (_, index) =>
            createBird(index)
        );

        createdBirds.sort((a, b) => a.depth - b.depth);

        setBirds(createdBirds);

        // Start animations with staggered timing
        createdBirds.forEach((bird, index) => {
            setTimeout(() => {
                if (isComponentMounted.current && isScreenFocused.current) {
                    startBirdAnimation(bird);
                }
            }, index * 1200);
        });

        return () => {
            frameIntervalsRef.current.forEach(interval => clearInterval(interval));
        };
    }, [numberOfBirds, createBird, startBirdAnimation]);

    // Frame animation using intervals (simpler approach)
    useEffect(() => {
        frameIntervalsRef.current.forEach(interval => clearInterval(interval));
        frameIntervalsRef.current = [];

        const intervals = birds.map((bird) => {
            return setInterval(() => {
                if (isComponentMounted.current && isScreenFocused.current) {
                    bird.frameIndex = (bird.frameIndex + 1) % 4;
                    // Force re-render by updating state
                    setBirds(prevBirds => [...prevBirds]);
                }
            }, bird.frameRate);
        });

        frameIntervalsRef.current = intervals;

        return () => {
            intervals.forEach(clearInterval);
        };
    }, [birds.length]);

    const playRandomSound = useCallback(async (bird: Bird) => {
        if (sounds.length > 0 && isComponentMounted.current) {
            try {
                const soundIndex = bird.spriteIndex % sounds.length;
                const randomSound = sounds[soundIndex];

                await randomSound.setPositionAsync(0);
                await randomSound.playAsync();

                setTimeout(() => {
                    randomSound.pauseAsync().catch(() => {});
                }, 1000 + Math.random() * 2000);
            } catch (error) {
                console.warn("Error playing bird sound:", error);
            }
        }
    }, [sounds]);

    const handleBirdTouch = useCallback((bird: Bird) => {
        playRandomSound(bird);
    }, [playRandomSound]);

    // Render birds
    const renderBird = (bird: Bird) => {
        const animatedStyle = useAnimatedStyle(() => {
            return {
                transform: [
                    { translateX: bird.x.value },
                    {
                        translateY: bird.y.value + bird.waveY.value + (windOffset.value * bird.windSensitivity * 15)
                    },
                    { scale: bird.scale },
                ],
                opacity: bird.opacity,
                zIndex: bird.zIndex,
            };
        });

        return (
            <TouchableWithoutFeedback
                key={bird.id}
                onPress={() => handleBirdTouch(bird)}
                hitSlop={{ top: 30, bottom: 30, left: 30, right: 30 }}
            >
                <Animated.View style={[styles.bird, animatedStyle]}>
                    <View style={[
                        styles.frame,
                        {
                            width: 16 * bird.scale,
                            height: 16 * bird.scale,
                            shadowColor: '#000',
                            shadowOffset: { width: 0, height: bird.depth * 3 },
                            shadowOpacity: bird.depth * 0.2,
                            shadowRadius: bird.depth * 4,
                            elevation: bird.depth * 3,
                        }
                    ]}>
                        <Animated.Image
                            source={bird.sprite}
                            style={{
                                width: 64 * bird.scale,
                                height: 64 * bird.scale,
                                transform: [
                                    { translateX: -bird.frameIndex * 16 * bird.scale },
                                    { translateY: 0 },
                                    { scaleX: -1 },
                                ],
                            }}
                            resizeMode="cover"
                        />
                    </View>
                </Animated.View>
            </TouchableWithoutFeedback>
        );
    };

    return (
        <View style={StyleSheet.absoluteFillObject} pointerEvents="box-none">
            {birds.map(renderBird)}
        </View>
    );
};

const styles = StyleSheet.create({
    bird: {
        position: 'absolute',
    },
    frame: {
        overflow: 'hidden',
    },
});

export default EnhancedBirdAnimation;